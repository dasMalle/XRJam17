namespace Mapbox.Unity.MeshGeneration.Modifiers.MeshModifiers
{
	using Mapbox.Unity.MeshGeneration.Components;
	using UnityEngine;

	[CreateAssetMenu(menuName = "Mapbox/Modifiers/Spawn In Mesh Modifier")]
	public class SpawnInMeshModifier : GameObjectModifier
	{
		[SerializeField]
		...
		public override void Run(FeatureBehaviour fb)
		{
			...
			while (_spawnedCount < spawnCount)
			{
				...
				if (Physics.Raycast(ray, out hit, 150, _layerMask))
				{
					...
					var transform = Instantiate(_prefabs[index], fb.transform, false).transform;
